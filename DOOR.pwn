...
Doors_Updatelabel(id)
{
	if(id != -1)
	{
		...
		if(dData[id][dGarage] == 1)
		{
			...
			dData[id][dPickupext] = CreateDynamicPickup(19197, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint]);
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			if(dData[id][dGarage] == 1)
			{
				...
				dData[id][dPickupint] = CreateDynamicPickup(19197, 23, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], dData[id][dIntvw], dData[id][dIntint], -1, 7);
			}
			else
			{
				new mstr[512];
...
CMD:createdoor(playerid, params[])
{
	...
	dData[did][dPickupext] = CreateDynamicPickup(19197, 23, dData[did][dExtposX], dData[did][dExtposY], dData[did][dExtposZ], dData[did][dExtvw], dData[did][dExtint]);
