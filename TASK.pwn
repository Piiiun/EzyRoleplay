...
{	
	...
	if(up_seconds == 60)
	{
	    ...
	    if(up_minutes == 60)
	    {
			...
			format(tstr, sizeof(tstr), "{B897FF}UPTIME: "WHITE_E"The server has been online for %s.", Uptime());
	    ...
	    if(AdvertData[i][advertTime] > 0)
	    {
	        ...
	        if(AdvertData[i][advertTime] <= 0)
	        {
				foreach(new  p: Player)
				{
					if(!pData[p][pTogAds])
					{
						SendClientMessageEx(p, COLOR_ORANGE2, "[ADS] "GREEN_E"%s", AdvertData[i][advertText]);
						SendClientMessageEx(p, COLOR_ORANGE2, "Phone Number: ["GREEN_E"%d"ORANGE_E2"] | Name: ["GREEN_E"%s"ORANGE_E2"]", AdvertData[i][advertNumber], AdvertData[i][advertName]);
					}
				}
