...
CMD:weapon(playerid, params[])
{
	...
	if(sscanf(params, "s[20]S()[128]", name, give))
		return SyntaxMsg(playerid, "/weapon [give/drop/pickup/pos/bone/hide]");
	...
	else if(!strcmp(name, "give", true))
	{
		...
		if(sscanf(give, "u", otherid))
			return SyntaxMsg(playerid, "/weapon [give] [playerid]");
	...
	else if(!strcmp(name, "drop", true))
	{
		...
		SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "{FFFFFF}ACTION: {DDA0DD}* %s takes out a %s and drops it on the floor.", ReturnName(playerid), ReturnWeaponName(weaponid));
	}
	else if(!strcmp(name, "pickup", true))
	{
		...
		if(wid != -1)
        {
			...
			SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "{FFFFFF}ACTION: {DDA0DD}* %s has picked up a %s.", ReturnName(playerid), ReturnWeaponName(DropWeap[wid][WeaponID]));
			ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1);
		}
	}
