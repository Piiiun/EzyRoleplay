...
CMD:createhouse(playerid, params[])
{
	...
	if(sscanf(params, "dd", price, type)) return SyntaxMsg(playerid, "/createhouse [price] [type, 1.small 2.medium 3.Big]");
...
CMD:gotohouse(playerid, params[])
{
	...
	if(sscanf(params, "d", hid))
		return SyntaxMsg(playerid, "/gotohouse [id]");
...
CMD:edithouse(playerid, params[])
{
    ...
    {
        SyntaxMsg(playerid, "/edithouse [id] [name]");
    ...
    else if(!strcmp(type, "locked", true))
    {
        ...
        if(sscanf(string, "d", locked))
            return SyntaxMsg(playerid, "/edithouse [id] [locked] [0/1]");
    ...
    else if(!strcmp(type, "price", true))
    {
        ...
        if(sscanf(string, "d", price))
            return SyntaxMsg(playerid, "/edithouse [id] [Price] [Amount]");
	...
	else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", htype))
            return SyntaxMsg(playerid, "/edithouse [id] [Type] [1.small 2.medium 3.Big]");
    ...
    else if(!strcmp(type, "owner", true))
    {
        ...
        if(sscanf(string, "s[32]", owners))
            return SyntaxMsg(playerid, "/edithouse [id] [owner] [player name] (use '-' to no owner)");
...
{
	new hid, otherid;
	if(sscanf(params, "ud", otherid, hid)) return SyntaxMsg(playerid, "/givehouse [playerid/name] [id] | /myhouse - for show info");
...
CMD:myhouse(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_HOUSES, DIALOG_STYLE_LIST, "{B897FF}Metro Roleplay{FFFFFF} - Houses", CMDSString, "Select", "Cancel");
	return 1;
}
