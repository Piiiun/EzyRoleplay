...
Locker_Refresh(lid)
{
    if(lid != -1)
    {
		...
		format(string, sizeof(string), "{FFFFFF}Press {A02BFC}[ALT]{FFFFFF} to Access Locker %d", lid, type);
		lData[lid][lPickup] = CreateDynamicPickup(1314, 23, lData[lid][lPosX], lData[lid][lPosY], lData[lid][lPosZ]+0.2, 0, lData[lid][lInt], _, 30);
		lData[lid][lLabel] = CreateDynamic3DTextLabel(string, COLOR_PURPLE, lData[lid][lPosX], lData[lid][lPosY], lData[lid][lPosZ]+0.5, 10.0);
	}
    return 1;
}
