...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 883.0904, -1800.1721, 13.7988)) return Error(playerid, "Kamu harus jual di tempat penjualan ikan.");
...
CMD:buybait(playerid, params[])
{
	...
	{
		if(pData[playerid][pWorm] >= 1000) return ErrorMsg(playerid, "Anda sudah membawa 1000 Umpan!");
		new mstr[128];
		format(mstr, sizeof(mstr), ""WHITE_E"Masukan jumlah Umpan:\n"WHITE_E"Umpan Price"GREEN_E"1$ / Gram");
		ShowPlayerDialog(playerid, DIALOG_UMPAN, DIALOG_STYLE_INPUT, "{B897FF}Metro Roleplay{FFFFFF} - Buy Umpan", mstr, "Buy", "Cancel");
	}
	return 1;
}
