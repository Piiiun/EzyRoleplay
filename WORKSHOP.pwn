...
CMD:createworkshop(playerid, params[])
{
	...
	new price;
	if(sscanf(params, "d", price)) return SyntaxMsg(playerid, "/createworkshop [price]");
	new totalcash[25];
...
CMD:gotoworkshop(playerid, params[])
{
	...
	if(sscanf(params, "d", wid))
		return SyntaxMsg(playerid, "/gotoworkshop [id]");
...
CMD:editworkshop(playerid, params[])
{
    ...
    {
        SyntaxMsg(playerid, "/editworkshop [id] [name]");
    ...
    else if(!strcmp(type, "status", true))
    {
        ...
        if(sscanf(string, "d", locked))
            return SyntaxMsg(playerid, "/editWorkshop [id] [locked] [0/1]");
    ...
    else if(!strcmp(type, "price", true))
    {
        ...
        if(sscanf(string, "d", price))
            return SyntaxMsg(playerid, "/editWorkshop [id] [Price] [Amount]");
	...
	else if(!strcmp(type, "money", true))
    {
        ...
        if(sscanf(string, "d", money))
            return SyntaxMsg(playerid, "/editWorkshop [id] [money] [Ammount]");
	...
	else if(!strcmp(type, "comp", true))
    {
        ...
        if(sscanf(string, "d", amount))
            return SyntaxMsg(playerid, "/editWorkshop [id] [Comp] [Ammount]");
    ...
    else if(!strcmp(type, "mat", true))
    {
        ...
        if(sscanf(string, "d", amount))
            return SyntaxMsg(playerid, "/editWorkshop [id] [mat] [Ammount]");
    ...
    else if(!strcmp(type, "owner", true))
    {
        ...
        if(sscanf(string, "s[24]", owners))
            return SyntaxMsg(playerid, "/editWorkshop [id] [owner] [player name] (use '-' to no owner)");
