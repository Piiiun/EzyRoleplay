...
CMD:report(playerid, params[])
{
    ...
    if(isnull(params))
    {
        SyntaxMsg(playerid, "/report [alasan]");
...
CMD:ar(playerid, params[])
{
    ...
    if(isnull(params))
        return SyntaxMsg(playerid, "/ar [report id] (/reports for a list)");
...
CMD:dr(playerid, params[])
{
    ...
    if(sscanf(params,"ds[32]", reportid, msg))
        return SyntaxMsg(playerid, "/dr [report id] [reason] (/reports for a list)");
